...
def get_label(song):
    ...
    albumartist = song.get('albumArtist').encode("utf-8")
    album_count = album_list.get(song.get('albumId'))  # TODO inconsistent?
    if album_count == 999:
        album_count = ' UsrDef'
    matched = 'Y' if song.get('nid') else 'N'
    ...
    label = "{:2}-{:02d} | {:45s} | {:12s} |  {}  | {:7} | {:6} |  {}".format(
        discnum, tracknum, album[:45], albumartist[:12], matched, album_count, length, playcount)
...
for item in sorted(dup_songs, key=lambda item: dup_songs[item][0].get('artist'), reverse=False):
    ...
    title = ("\n\n\n" +
             ...
             item +
             "\n\n" +
             "   D-T  | {:45s} | {:12s} | nid | MySongs | Length | Play Count ").format("Album", "Album Artist")
